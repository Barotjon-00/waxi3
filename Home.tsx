...
const heroSlides = [
  ...
  { src: "https://images.unsplash.com/photo-1522673607200-164d1b6ce486?auto=format&fit=crop&w=1100&q=85", alt: "Пара идёт навстречу друг другу", caption: "всегда навстречу" },
...
export default function Home() {
  ...
  return (
    <main className="love-page">
      ...
      <section className="moments-section"><div className="section-heading"><span className="eyebrow">наша хроника</span><span className="section-number">03 / 06</span></div><div className="timeline">{moments.map((moment, i) => <article className="timeline-item" key={moment.date}><div className="timeline-marker"><span>{moment.date}</span><i /></div><div><p className="moment-label">{moment.label}</p><p className="moment-text">{moment.text}</p></div><span className="moment-heart"><Heart size={16} strokeWidth={1.5} /></span></article>)}</div></section>
